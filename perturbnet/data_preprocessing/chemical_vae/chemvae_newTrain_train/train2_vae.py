...
def vectorize_data(params):
    ...
    print('Vectorization...')
    print(params['X_save_file'])
    print(params['X_save_file_time'])
    begin = time.time()
    X = mu.smiles_to_hot(smiles, MAX_LEN, params[
                             'PADDING'], CHAR_INDICES, NCHARS)
    end = time.time() - begin
    np.save(params['X_save_file'], X)
    np.save(params['X_save_file_time'], np.array([end]))
...
def main_no_prop(params):
    ...
    varlayer.save(params['varlayer_weights_file'])
...
def main_property_run(params):
    ...
    # load full models:
    AE_only_model, AE_PP_model, encoder, decoder, varlayer, property_predictor, kl_loss_var = load_models(params)
