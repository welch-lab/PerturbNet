...
def decoder_model(params):
    ...
    if params['gru_depth'] > 1:
        ...
        if params['do_tgru']:
            x_out = TerminalGRU(units=params['NCHARS'],
...
#
#    z_mean = Input(shape=(params['hidden_dim'],),  name='z_mean')
#    middle = Input(shape=(int(params['hidden_dim'] *
#                           params['hg_growth_factor'] ** (params['middle_layer'] - 1)),),  name='middle')
#
#    z_log_var = Dense(params['hidden_dim'], name='z_log_var_sample')(middle)
#     
...
#
#    return Model([z_mean, middle], [z_samp, z_mean_log_var_output], name="varLayer")
...
def varLayer(params):
	...
	return Model([z_mean, middle], [z_log_var, z_mean_log_var_output], name = 'varLayer')



def load_varLayer(params):
    return load_model(params['varlayer_weights_file'])
